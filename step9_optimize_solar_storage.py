...
optimization problem over (solar_kw, battery_kwh). SciPy is required; the program
will fail with a clear error if SciPy is not installed.
...
- Required library: SciPy (scipy.optimize) for differential_evolution + Powell.
...
def _scipy_optimize(plan: str, ctx: CountyContext) -> Tuple[List[float], float]:
    try:
        from scipy.optimize import differential_evolution, minimize
    except Exception as e:
        raise ImportError(
            "SciPy is required for step9_optimize_solar_storage. Install with 'pip install scipy'."
        ) from e

    bounds = [ctx.bounds.solar_kw, ctx.bounds.battery_kwh]
    # Global search with differential evolution, limited iterations for time
...
def optimize_for_plan(plan: str, ctx: CountyContext) -> Tuple[List[float], float]:
    # SciPy-only optimization; raise if SciPy is unavailable
    return _scipy_optimize(plan, ctx)
