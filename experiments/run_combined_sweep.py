...
try:
    from .combined_sweep import run, CombinedSweepOptions  # type: ignore
except Exception:
    import sys
...
def parse_args():
    p = argparse.ArgumentParser(description="Run combined PV fraction × Battery capacity sweeps (non-intrusive)")
    p.add_argument("--base-input-dir", default="data/loadprofiles")
    p.add_argument("--experiments-root", default="data/experiments/combined_size_sweep")
