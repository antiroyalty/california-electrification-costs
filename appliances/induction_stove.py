"""
Induction stove appliance class for residential electrification cost modeling.

This module defines the InductionStoveAppliance class used to model the capital costs,
lifetime, and incentives for induction cooking systems that replace gas stoves
...
class InductionStoveAppliance(ElectricAppliance):
    """
    Class representing induction stove appliances for home electrification.
    
    This class models the capital costs, lifetime, and incentives for induction
    cooking systems that replace gas stoves in residential electrification scenarios.
    """
    
    def __init__(self, 
                 stove_type: str = "induction",
                 base_cost: float = 2000.0,
                 lifetime_years: int = 15):
        """
        Initialize induction stove appliance.
        
        Args:
            stove_type: Type of electric cooking system (default: "induction")
            base_cost: Base equipment and installation cost in dollars
            lifetime_years: Expected equipment lifetime in years
        """
        super().__init__(f"electric_{stove_type}_stove", base_cost, lifetime_years)
    ...
    def _add_default_incentives(self) -> None:
        """Add default federal incentives for induction stoves."""
        # Federal tax credit for induction stoves (Inflation Reduction Act)
        federal_credit = Incentive(
            name="Federal Residential Electrification Rebate",
            value=420.0,
            unit="$",
            description="Federal rebate for residential induction stoves under Inflation Reduction Act",
    ...
    def get_cost_breakdown(self, scenario: IncentiveScenario = IncentiveScenario.FULL_INCENTIVES) -> Dict:
        """Return detailed cost breakdown for induction stove appliance."""
        incentives_detail = []
