...
- Required: `scipy` (`scipy.optimize`) for global + local continuous optimization. The optimizer will fail with a clear error if SciPy is not installed.
...
- Required library
  - SciPy (`scipy.optimize`) for global + local continuous optimization; the optimizer exits with an error if missing.
