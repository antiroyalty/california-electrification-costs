...
def generate_diff_html(merged_gdf, output_path, rate):
    ...
    filename = os.path.join(output_path, f"difference_map_{rate}.html")
...
def process(base_input_dir, base_output_dir, housing_type, counties, baseline_scenario, electrified_scenario):
    # For differences, we only care about the gap between baseline and heat pump.
    # All arguments are passed in, and we assume that the housing type remains consistent.
    process_difference_map(base_input_dir, base_output_dir, housing_type, counties, baseline_scenario, electrified_scenario)
...
process(base_input_dir, base_output_dir, housing_type, counties, "baseline", "heat_pump")
